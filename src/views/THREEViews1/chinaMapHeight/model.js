...
import { createExtrdue } from './createExtrdue'
    ...
    features.forEach((ele) => {
        ...
        if (geometry.type === 'Polygon') {
            geometry.coordinates.forEach((ele) => {
                ...
                const h = obj[name] / 10500
                //调用创建形状的方法创建形状
                const shape = createExtrdue(pointArr,h)
                const line=createLine(lineArr)
                line.position.z += h + 0.5
                const label=createLabel(name,center[0],center[1],h)
                //形状加入层级模型
