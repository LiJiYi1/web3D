...
// out_FragColor=vec4(1.0,1.0,1.0,0.5);

// out_FragColor.rgb = mix( back.xyz, col.xyz, col.a );
out_FragColor.rgb = mix( out_FragColor.rgb, col.xyz, col.a );
// out_FragColor += col;
// out_FragColor.rgb *= pow(16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y),0.1);

}
`;
function AddCircleScanPostStage(viewer, maxRadius, scanColor, duration) {
...
function addCircleScan(viewer,data){
    ...
    return  AddCircleScanPostStage(viewer, data.r,data.scanColor,data.interval);
}

const redSphere = viewer.entities.add({
    name : 'Red sphere with black outline',
    position: Cesium.Cartesian3.fromDegrees(117.217124,31.808777, 0.0),
    ellipsoid : {
